...
- (UILabel *)zWTitleLabel {
    if (!_zWTitleLabel) {
        ...
        _zWTitleLabel.font = [UIFont systemFontOfSize:S_RatioW(16.f)];
    }
...
/// 背景的透明度
static CGFloat const kAlpha  = 0.5f;
...
- (instancetype)initWithActionTitle:(NSString *)title action:(NSArray<NSString *> *)titleArray {
    ...
    if (self) {
       ...
       CGFloat actionTitleHeight = [title boundingRectWithSize:CGSizeMake(WIDTH - 30, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:16.f]} context:nil].size.height;
...
//    window.windowLevel = UIWindowLevelStatusBar;
    ...
    [UIView animateWithDuration:kAnimateTime animations:^{
        ...
        self.actionSheetTableView.frame = CGRectMake(0, HEIGHT - CellHeight - kSpacing - self->_tableViewHeight, WIDTH, self->_tableViewHeight);
        self.cancleButton.frame = CGRectMake(0, HEIGHT - CellHeight, WIDTH, CellHeight);
        self.titleLabel.frame = CGRectMake(0, HEIGHT  - CellHeight - kSpacing  - self->_tableViewHeight - 0.5 - self->_titleHeight, WIDTH, self->_titleHeight);
    }];
}
...
- (void)hiddenActionSheet {
    [UIView animateWithDuration:kAnimateTime animations:^{
        ...
        self.actionSheetTableView.frame = CGRectMake(0, self->_tebleViewY, WIDTH,self->_tableViewHeight);
        self.cancleButton.frame = CGRectMake(0, self->_tebleViewY + kSpacing + self->_tableViewHeight, WIDTH, CellHeight);
        self.titleLabel.frame = CGRectMake(0, HEIGHT, WIDTH, self->_titleHeight);
...
- (UIButton *)cancleButton {
    if (!_cancleButton) {
        ...
        _cancleButton.titleLabel.font = [UIFont systemFontOfSize:S_RatioW(16.f)];
