...
#define FTDefaultMenuIconSize               20.f
#define FTDefaultMenuRowHeight              14.f
...
static NSString  *const HQJPopMenuTableViewCellIndentifier = @"FTPopOverMenuTableViewCellIndentifier";
static NSString  *const HQJPopMenuImageCacheDirectory = @"com.FTPopOverMenuImageCache";
/**
 *  FTPopOverMenuArrowDirection
 */
typedef NS_ENUM(NSUInteger, HQJPopMenuArrowDirection) {
    /**
     *  Up
     */
    HQJPopMenuArrowDirectionUp,
    /**
     *  Down
     */
    HQJPopMenuArrowDirectionDown,
};

#pragma mark - HQJPopMenuModel

@implementation HQJPopMenuModel
...
#pragma mark - HQJPopMenuConfiguration

@interface HQJPopMenuConfiguration ()

@end

@implementation HQJPopMenuConfiguration

+ (HQJPopMenuConfiguration *)defaultConfiguration {
    HQJPopMenuConfiguration *configuration = [[HQJPopMenuConfiguration alloc] init];
...
#pragma mark - HQJPopMenuCell

@interface HQJPopMenuCell ()
...
- (void)setupWithMenuName:(NSString *)menuName menuImage:(id)menuImage selected:(BOOL)selected configuration:(HQJPopMenuConfiguration *)configuration {
...
- (NSString *)filePathForImageURL:(NSURL *)url {
    NSString *diskCachePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:HQJPopMenuImageCacheDirectory];
...
#pragma mark - HQJPopMenuView

@interface HQJPopMenuView () <UITableViewDelegate,UITableViewDataSource>

@property (nonatomic, strong) UITableView *menuTableView;
@property (nonatomic, strong) NSArray *menuStringArray;
@property (nonatomic, strong) NSArray *menuImageArray;
@property (nonatomic, assign) HQJPopMenuArrowDirection arrowDirection;
@property (nonatomic, strong) HQJPopMenuDoneBlock doneBlock;
@property (nonatomic, strong) CAShapeLayer *backgroundLayer;
@property (nonatomic, strong) HQJPopMenuConfiguration *config;

@end

@implementation HQJPopMenuView
...
- (HQJPopMenuConfiguration *)config {
    if (!_config) {
        _config = [HQJPopMenuConfiguration defaultConfiguration];
    }
    return _config;
}
...
- (void)showWithFrame:(CGRect )frame
     ...
     shouldAutoScroll:(BOOL)shouldAutoScroll
               config:(HQJPopMenuConfiguration *)config
       arrowDirection:(HQJPopMenuArrowDirection)arrowDirection
            doneBlock:(HQJPopMenuDoneBlock)doneBlock {
    self.frame = frame;
    self.config = config ? config : [HQJPopMenuConfiguration defaultConfiguration];
    ...
    if (_arrowDirection == HQJPopMenuArrowDirectionDown) {
...
- (void)drawBackgroundLayerWithAnglePoint:(CGPoint)anglePoint {
    ...
    switch (_arrowDirection) {
        case HQJPopMenuArrowDirectionUp:{
        ...
        case HQJPopMenuArrowDirectionDown:{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([object isKindOfClass:[HQJPopMenuModel class]]) {
        title = ((HQJPopMenuModel *)object).title;
        menuImage = ((HQJPopMenuModel *)object).image;
        selected = ((HQJPopMenuModel *)object).selected;
    }else{
        title = [NSString stringWithFormat:@"%@", object];
    }
    
    HQJPopMenuCell *menuCell = [[HQJPopMenuCell alloc]initWithStyle:UITableViewCellStyleDefault
                                                          reuseIdentifier:HQJPopMenuTableViewCellIndentifier
                                                                 menuName:title
                                                                menuImage:menuImage
                                                                 selected:selected
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id object = _menuStringArray[indexPath.row];
    if ([object isKindOfClass:[HQJPopMenuModel class]]) {
        [_menuStringArray enumerateObjectsUsingBlock:^(HQJPopMenuModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
@interface HQJMenu () <UIGestureRecognizerDelegate>

@property (nonatomic, strong) UIView * backgroundView;
@property (nonatomic, strong) HQJPopMenuView *popMenuView;
@property (nonatomic, strong) HQJPopMenuDoneBlock doneBlock;
@property (nonatomic, strong) HQJPopMenuDismissBlock dismissBlock;
...
@implementation HQJMenu

+ (HQJMenu *)sharedInstance {
    static dispatch_once_t once = 0;
    static HQJMenu *shared;
    dispatch_once(&once, ^{ shared = [[HQJMenu alloc] init]; });
    return shared;
}
...
- (HQJPopMenuView *)popMenuView {
    if (!_popMenuView) {
        _popMenuView = [[HQJPopMenuView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
...
- (void)adjustPopOverMenu {
    ...
    HQJPopMenuArrowDirection arrowDirection;
    
    if (senderRect.origin.y + senderRect.size.height/2  < KSCREEN_HEIGHT/2) {
        arrowDirection = HQJPopMenuArrowDirectionUp;
        menuArrowPoint.y = 0;
    }else{
        arrowDirection = HQJPopMenuArrowDirectionDown;
    ...
    if (arrowDirection == HQJPopMenuArrowDirectionUp) {
...
- (void)prepareToShowWithMenuRect:(CGRect)menuRect menuArrowPoint:(CGPoint)menuArrowPoint shouldAutoScroll:(BOOL)shouldAutoScroll arrowDirection:(HQJPopMenuArrowDirection)arrowDirection {
    CGPoint anchorPoint = CGPointMake(menuArrowPoint.x/menuRect.size.width, 0);
    if (arrowDirection == HQJPopMenuArrowDirectionDown) {
