...
//        }
//    }];
    [[NSUserDefaults standardUserDefaults] setObject:@"连接中" forKey:@"BluetoothState"];
    [[JWBluetoothManage sharedInstance] autoConnectLastPeripheralCompletion:^(CBPeripheral *perpheral, NSError *error) {
        if (!error) {
            ...
            [[[NSUserDefaults alloc] initWithSuiteName:@"group.com.first.HQJBusiness"] setObject:@"连接成功" forKey:@"BluetoothState"];
            dispatch_async(dispatch_get_main_queue(), ^{
                //                [self.tableView reloadData];
            });
        }else{
            [[[NSUserDefaults alloc] initWithSuiteName:@"group.com.first.HQJBusiness"] setObject:@"连接失败" forKey:@"BluetoothState"];
...
- (void)printe{
    ...
    NSLog(@"模型数组哦：%@",self.pushModel.list);
