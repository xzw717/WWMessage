...
#define ZGMainThreadAssert() NSAssert([NSThread isMainThread], @"ZGProgressHUD needs to be accessed on the main thread.");

CGFloat const ZGProgressMaxOffset = 1000000.f;

static const CGFloat ZGDefaultPadding = 19.f;
static const CGFloat ZGDefaultLabelFontSize = 16.f;
static const CGFloat ZGDefaultDetailsLabelFontSize = 12.f;
...
@interface ZGProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ZGProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ZGProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (ZGProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (ZGProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = ZGProgressHUDAnimationFade;
    _mode = ZGProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(ZGProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == ZGProgressHUDAnimationZoom) {
        type = animatingIn ? ZGProgressHUDAnimationZoomIn : ZGProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == ZGProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == ZGProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == ZGProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == ZGProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    ZGProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<ZGProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    ZGBackgroundView *backgroundView = [[ZGBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = ZGProgressHUDBackgroundStyleSolidColor;
    ...
    ZGBackgroundView *bezelView = [ZGBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:ZGDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:ZGDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [ZGProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:ZGDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[ZGRoundProgressView class]];

    ZGProgressHUDMode mode = self.mode;
    ...
    else if (mode == ZGProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[ZGBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == ZGProgressHUDModeDeterminate || mode == ZGProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[ZGRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == ZGProgressHUDModeAnnularDeterminate) {
            [(ZGRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == ZGProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == ZGProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[ZGProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[ZGProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[ZGRoundProgressView class]]) {
        ZGRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ZGRoundProgressView appearanceWhenContainedIn:[ZGProgressHUD class], nil];
#else
        appearance = [ZGRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[ZGProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((ZGRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((ZGRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[ZGBarProgressView class]]) {
        ZGBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ZGBarProgressView appearanceWhenContainedIn:[ZGProgressHUD class], nil];
#else
        appearance = [ZGBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[ZGProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((ZGBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((ZGBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    ZGBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? ZGDefaultPadding : 0.f;
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == ZGProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation ZGRoundProgressView
...
@implementation ZGBarProgressView
...
@implementation ZGBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = ZGProgressHUDBackgroundStyleBlur;
            ...
            _style = ZGProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(ZGProgressHUDBackgroundStyle)style {
    if (style == ZGProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = ZGProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    ZGProgressHUDBackgroundStyle style = self.style;
    if (style == ZGProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == ZGProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation ZGProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [ZGProgressHUD allHUDsForView:view];
    for (ZGProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable ZGProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    ZGBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == ZGProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    ZGMainThreadAssert();
    self.backgroundView.style = ZGProgressHUDBackgroundStyleSolidColor;
...
@implementation ZGProgressHUDRoundedButton
