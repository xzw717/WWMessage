...
- (void)setText:(NSString *)text maxChar:(int)maxChar
{
    ...
    if (data.length > maxChar) {
        NSInteger a = data.length / maxChar;
        if (maxChar * a != data.length) {
            a = a + 1;

        }
       
        for (NSInteger i = 0; i < a; i ++) {
            NSData *data1 = [[text dataUsingEncoding:enc] subdataWithRange:NSMakeRange(maxChar * (i + 1), data.length - maxChar * (i + 1))];
            NSString *text1 = [[NSString alloc] initWithData:data1 encoding:enc];
        
           NSData *data2 = [data subdataWithRange:NSMakeRange( i * maxChar , maxChar * (i + 1))];
            NSString *text2 = [[NSString alloc] initWithData:data2 encoding:enc];
//            if (a == 0) {
                [str appendFormat:@"%@", [NSString stringWithFormat:@"%@\n",text2]];
                [str appendFormat:@"%@",text1];
//                text = [text2 stringByAppendingString:@"\n"];
//                text = [text stringByAppendingString:text1];

//            } else {
//                [str appendFormat:@"%@", [NSString stringWithFormat:@"%@\n",str]];
//                [str appendFormat:@"%@%@", str,text1];
//            }
...
- (void)hqj_setText:(NSString *)text
{
    NSStringEncoding enc = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
    NSData *data = [text dataUsingEncoding:enc];
    if (data.length > 12) {
        ...
        text = [[NSString alloc] initWithData:data encoding:enc];
        if (!text) {
            data = [data subdataWithRange:NSMakeRange(0, 12 - 1)];
            text = [[NSString alloc] initWithData:data encoding:enc];
        }
...
- (void)hqj_setOffset:(NSInteger)offset
{
    NSInteger remainder = offset % 256;
    NSInteger consult = offset / 256;
    Byte spaceBytes2[] = {0x1B, 0x24,consult , remainder};
    [_printerData appendBytes:spaceBytes2 length:sizeof(spaceBytes2)];
}
...
//    [self appendText:text alignment:alignment fontSize:HLFontSizeTitleSmalle];
    [self hqj_appendText:text alignment:alignment fontSize:HLFontSizeTitleSmalle];
}
...
- (void)hqj_appendText:(NSString *)text alignment:(HLTextAlignment)alignment fontSize:(HLFontSize)fontSize
{
    // 1.文字对齐方式
    [self setAlignment:alignment];
    // 2.设置字号
...
- (void)appendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle
{
    ...
    if (left) {
//        [self setText:left maxChar:12];
        [self setSubText:[self dealWithString:left]];
//        [self setAlignment:HLTextAlignmentLeft];

    }
    
    if (middle) {
//        [self hqj_setOffset:150 + offset];
...
- (void)hqj_appendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right
{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
